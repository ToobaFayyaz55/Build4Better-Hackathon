  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Text
            style={[styles.tabText, activeTab === "my" && { color: "#fff" }]}
          >
            My Equipment
          </Text>
        </TouchableOpacity>
        ...
        onSave={(newData) => {
          if (editingEquipment) {
            setEquipmentList((prev) =>
              prev.map((e) => (e.id === editingEquipment.id ? newData : e))
            );
          } else {
            setEquipmentList((prev) => [
              ...prev,
              { ...newData, id: prev.length + 1, owner_id: 1 },
            ]);
          }
      ...
      {/* Floating Add Button */}
      <TouchableOpacity style={styles.addBtn} onPress={() => openAddEdit(null)}>
...
const styles = StyleSheet.create({
  ...
  badgeText: { color: "#fff", fontSize: 10 }, // ---- Tabs ----
  ...
  tab: { flex: 1, padding: 10, alignItems: "center" }, // Using raw hex for PRIMARY color
  tabActive: { backgroundColor: "#bd9e4b" },
  tabText: { color: "#374151", fontWeight: "600", fontSize: 14 },
  tabTextActive: { color: "#fff" }, // ---- Scroll / List ----

  scroll: { flex: 1 },
  listContainer: { paddingHorizontal: 16, paddingBottom: 120 }, // ---- Floating Add Button ----

  addBtn: {
    position: "absolute",
    bottom: 24,
    right: 24, // Using raw hex for PRIMARY color
