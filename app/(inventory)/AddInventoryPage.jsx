...
const crop_batches = [
    ...
    { id: 2, crop_id: 1, qty: 10, harvest_date: "2025-10-20", expiry_date: "2025-11-07", sold_qty: 1, status: "Available" },
  { id: 3, crop_id: 2, qty: 100, harvest_date: "2025-10-10", expiry_date: "2025-11-20", sold_qty: 80, status: "Expiring Soon" },
  { id: 4, crop_id: 3, qty: 40, harvest_date: "2025-10-25", expiry_date: "2025-11-10", sold_qty: 40, status: "Sold Out" },
];
...
export default function InventoryScreen() {
  ...
  const cropSummaries = useMemo(() => {
    const map = {};
    crops.forEach((c) => {
      const batches = crop_batches.filter((b) => b.crop_id === c.id);
      ...
      const overallPercentRemaining = totalQty === 0 ? 0 : (remaining / totalQty) * 100;
      map[c.id] = { totalQty, totalBatches: batches.length, counts, overallPercentRemaining, batches };
    });
    return map;
  }, []);
  ...
  const displayedCrops = crops.filter((c) => {
    ...
    if (activeFilterKey === "near") return (summary.counts["Expiring Soon"] || 0) > 0;
    if (activeFilterKey === "sold") return (summary.counts["Sold Out"] || 0) > 0;
    return true;
  });
  ...
  const selectedBatches = selectedCropId ? cropSummaries[selectedCropId]?.batches || [] : [];
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <View>
          ...
          <Text style={styles.countText}>{crops.length} crops • {crop_batches.length} batches</Text>
        </View>

        <TouchableOpacity style={[styles.addBtn, { backgroundColor: PRIMARY }]} onPress={() => setShowModal(true)}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        {displayedCrops.map((c) => {
          const summary = cropSummaries[c.id] || { totalQty: 0, totalBatches: 0, counts: {}, overallPercentRemaining: 0, batches: [] };
          return (
            <CropCard
              key={c.id}
              crop={c}
        ...
        {displayedCrops.length === 0 && (
          <View style={styles.empty}>
            <Text style={styles.emptyText}>No crops found matching filters.</Text>
          </View>
        )}
      </ScrollView>

      {/* Add batch modal (your existing component) */}
      <AddItemModal visible={showModal} onClose={() => setShowModal(false)} />
...
const styles = StyleSheet.create({
  ...
  sep: { height: 8, backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "#F3F4F6" },
